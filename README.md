...
- `Edit task` otwiera osobny modal (nie ten sam formularz co przy dodawaniu nowego wpisu) – edycja opisu i czasu trwania jest zaimplementowana względem realnej struktury tego modala, ale **projekt i tagi nie są tam edytowane** (ryzyko wymieszania starych i nowych wartości bez potwierdzonego sposobu czyszczenia istniejącego zaznaczenia) – zmiana samego projektu/tagów będzie bezpiecznie, ale bezskutecznie, próbowana ponownie przy każdym kolejnym sync.
- `Delete task` wciąż niezweryfikowane – nie wiadomo, czy usuwanie pyta o potwierdzenie, a obecny kod szuka przycisku potwierdzenia przez `get_by_role("button", ...)`, co prawdopodobnie nigdy nie trafi w nic, bo przyciski Quidlo (patrz modal edycji) to zwykłe `<div>` bez roli ARIA. **Obejrzyj usuwanie na żywo przed zaufaniem mu na realnych danych.**
